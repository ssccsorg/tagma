...
impl CoordSet {
    ...
    pub fn retain<F: FnMut(&Coord) -> bool>(&mut self, mut f: F) {
        for i in 0..Coord::N_VALID {
            // i is always in 0..11172 (Coord::N_VALID), so Coord::new never fails.
            if let Some(coord) = Coord::new(i as u16) {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while self.word_idx < CoordSet::WORD_COUNT {
            ...
            if w != 0 {
                ...
                self.bits[self.word_idx] = w & (w - 1);
                // idx is always < Coord::N_VALID (11172) because only valid
                // Coords are inserted. The last word has unused bits beyond
                // 11172; they are never set.
                if let Some(coord) = Coord::new(idx as u16) {
                    return Some(coord);
                }
                // Fallback: skip invalid coord and continue scanning.
                self.word_idx += 1;
            }
            self.word_idx += 1;
        }
